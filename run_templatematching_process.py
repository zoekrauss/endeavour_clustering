...
def day_detect(day1,tribe):
    ...
    #party=tribe.client_detect(client,t1,t2,threshold=3.2, threshold_type='absolute',trig_int=1,save_progress=False,ignore_bad_data=True,parallel_process=False)
        #party=tribe.client_detect(client,t1,t2,threshold=2.5, threshold_type='absolute',trig_int=1,save_progress=False,ignore_bad_data=True,parallel_process=False)
    party=tribe.client_detect(client,t1,t2,threshold=0.7, threshold_type='av_chan_corr',trig_int=1,save_progress=False,ignore_bad_data=True,parallel_process=False)
    fbase = 'detections_sep2017_enwf/'
...
def main():
    ...
    for i,temp in enumerate(tribe):
        tribe[i].st = tribe[i].st.select(station='ENWF')
    
    # Only keep templates that have picks on both channels
    keep = []
    for i,temp in enumerate(tribe):
        if len(tribe[i].st) > 1:
            keep.append(tribe[i])

    tribe = eqcorrscan.core.match_filter.tribe.Tribe(templates=keep)
