...
def day_detect(day1,tribe):
    ...
    party=tribe.client_detect(client,t1,t2,threshold=0.4, threshold_type='av_chan_corr',trig_int=1,save_progress=False,ignore_bad_data=True,parallel_process=False)
    fbase = 'allsep2017_detections/'
...
def main():
    ...
    t2 = datetime(2017,9,2)
    time_bins = pd.to_datetime(np.arange(t1,t2,pd.Timedelta(1,'days')))

    tribe = eqcorrscan.core.match_filter.tribe.read_tribe('all_templates_sep2017.tgz')
